...
// s3CleanPath removes all objects under a given S3 path prefix using the persistent
// s3-client pod. Idempotent — does nothing if the path doesn't exist.
func s3CleanPath(pathPrefix string) {
	_, _ = kubectlQ("exec", "-n", testNamespace, "s3-client", "--",
		"/bin/sh", "-c",
